...
def calculate_adx(highs: List[float], lows: List[float], closes: List[float], period: int = 14) -> List[float]:
    """
    Calculate Average Directional Index (ADX) / 計算平均趨向指數
    ...
    Args:
        ...
        lows: List of low prices / 最低價列表
        closes: List of closing prices / 收盤價列表
        period: ADX period (default: 14) / ADX 週期（預設：14）
        
    Returns:
        List of ADX values / ADX 值列表
    """
    if len(closes) < period + 1:
        return [0.0] * len(closes)
    ...
    plus_dm_smooth = [sum(plus_dm[1:period+1]) / period]
    minus_dm_smooth = [sum(minus_dm[1:period+1]) / period]
    tr_smooth = [sum(trs[:period]) / period]
    
    for i in range(period, len(plus_dm)-1):
        plus_dm_smooth.append((plus_dm_smooth[-1] * (period - 1) + plus_dm[i+1]) / period)
        minus_dm_smooth.append((minus_dm_smooth[-1] * (period - 1) + minus_dm[i+1]) / period)
        tr_smooth.append((tr_smooth[-1] * (period - 1) + trs[i]) / period)
    ...
    # Pad to match closes length
    pad = len(closes) - len(adx_values)
    result = [0.0] * pad + adx_values
    return result
