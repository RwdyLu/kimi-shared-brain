...
class StrategyValidationManager:
    ...
    def add_strategy(self, strategy_id: str, strategy_name: str, trial_days: int = 30) -> StrategyTrial:
        """
        ...
        self.logger.info(f"Added strategy {strategy_id} ({strategy_name}) " f"with {trial_days} day trial")
    ...
    def save_state(self, filepath: str = "state/strategy_validation.json"):
        ...
        state = {
            "strategies": {sid: trial.to_dict() for sid, trial in self.strategies.items()},
            "criteria": {
