...
class SymbolResult:
    ...
    # P3 Risk Management Indicators / P3 風險管理指標
...
class MonitorRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            use_genetic: Enable genetic strategy integration / 啟用基因策略整合
        """
        ...
        # If genetic enabled, load from merged genetic config
        self.strategy_executor = StrategyExecutor(use_genetic=use_genetic)
    ...
    def _save_indicator_snapshot(self, result: SymbolResult, run_id: Optional[int] = None) -> None:
        ...
        try:
            ...
            # Add P3 risk management indicators / 添加 P3 風險管理指標
            p3_fields = ["atr14", "adx14", "volume_ema20"]
    ...
    def _calculate_indicators(self, data_5m: List[KlineData], data_1m: List[KlineData]) -> Dict[str, Any]:
        ...
        # ATR14
        if len(closes_5m) >= 15:
            atr_list = calculate_atr(highs_5m, lows_5m, closes_5m, period=14)
            if atr_list:
                result["atr14"] = atr_list[-1]

        # ADX14
        if len(closes_5m) >= 20:
            adx_list = calculate_adx(highs_5m, lows_5m, closes_5m, period=14)
            if adx_list:
                result["adx14"] = adx_list[-1]

        # Volume EMA20 (for volume breakout strategies)
        volumes_5m = [k.volume for k in data_5m]
        if len(volumes_5m) >= 20:
            from indicators.calculator import calculate_ema
            vol_ema = calculate_ema(volumes_5m, period=20)
            if vol_ema:
    ...
    def run_for_symbol(self, symbol: str, run_id: Optional[int] = None) -> SymbolResult:
        ...
        market_data = {
            ...
            "volume": data_5m[-1].volume if data_5m else None,
            "volumes": [k.volume for k in data_5m] if data_5m else [],
        ...
        # Get enabled strategy IDs for filtering / 取得已啟用策略 ID 用於過濾
        enabled_ids = {s["id"] for s in self.strategy_executor.enabled_strategies}
        
        def _signal_strategy_id(s):
            meta = s.metadata or {}
            return meta.get("strategy_id", meta.get("strategy_name", ""))
        
        # Filter signals to only include enabled strategies / 只保留已啟用策略的訊號
        confirmed = [s for s in signals if s.level == SignalLevel.CONFIRMED and _signal_strategy_id(s) in enabled_ids]
        watch_only = [s for s in signals if s.level == SignalLevel.WATCH_ONLY and _signal_strategy_id(s) in enabled_ids]
        ...
        for strategy in self.strategy_executor.enabled_strategies:
            ...
            not_impl = sum(1 for r in results if r.result.value == 'not_implemented')
            
            status_icon = "✅" if passed == len(conditions) and len(conditions) > 0 else "❌"
            print(f"      {status_icon} {strat_name}: {passed}/{len(conditions)} passed, {failed} failed, {missing} missing, {not_impl} not_impl")
        ...
        result = SymbolResult(
            ...
            highs=[k.high for k in data_5m] if data_5m else None,
            lows=[k.low for k in data_5m] if data_5m else None,
