...
class StrategyRanking:
    ...
    def rank_strategies(self,
                       method: RankingMethod = RankingMethod.COMPOSITE) -> List[StrategyScore]:
        """
        Rank all strategies.
    ...
    def eliminate_weak(self, min_tier: str = 'C') -> List[StrategyScore]:
        ...
        self.logger.info(
            f"Eliminated {len(eliminated)} strategies below tier {min_tier}"
        )
        
        return eliminated
    
    def compare_strategies(self, 
                          strategy_ids: List[str]) -> Dict:
        """
...
if __name__ == "__main__":
    ...
    strategies = [
        StrategyScore("MA_10_50", "BTCUSDT", "4h", total_return=25, sharpe_ratio=1.5, max_drawdown=-8, win_rate=65, profit_factor=2.1),
        StrategyScore("RSI_14", "ETHUSDT", "1h", total_return=18, sharpe_ratio=1.2, max_drawdown=-12, win_rate=58, profit_factor=1.8),
        StrategyScore("SNR_v1", "BTCUSDT", "4h", total_return=30, sharpe_ratio=1.8, max_drawdown=-6, win_rate=70, profit_factor=2.5),
        StrategyScore("Bad_Strategy", "SOLUSDT", "1h", total_return=-5, sharpe_ratio=-0.3, max_drawdown=-25, win_rate=35, profit_factor=0.8),
    ]
