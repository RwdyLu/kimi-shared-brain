...
class OptimisticLock:
    ...
    def acquire(self, expected_version: int) -> LockResult:
        ...
        with self._lock:
            ...
            return LockResult(success=True, lock_type=LockType.OPTIMISTIC, version=current_version + 1)
...
class AdaptiveLock:
    ...
    def acquire(self, expected_version: Optional[int] = None, timeout: float = 30.0) -> LockResult:
        """
...
class RetryWithBackoff:
    ...
    def __init__(
        self, max_retries: int = 3, base_delay: float = 1.0, max_delay: float = 30.0, exponential: bool = True
    ):
...
def with_optimistic_lock(task_id: str, expected_version: int, operation: Callable) -> Any:
    """
...
def with_pessimistic_lock(task_id: str, operation: Callable, timeout: float = 30.0) -> Any:
    """
...
def with_retry(operation: Callable, max_retries: int = 3, *args, **kwargs) -> Any:
    """
    Execute operation with retry logic.
