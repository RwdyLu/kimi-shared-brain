...
class MonitoringScheduler:
    ...
    def __init__(
        ...
        Args:
            ...
            trade_executor: Trade executor for paper trading
            use_genetic: Enable genetic strategy integration / 啟用基因策略整合
        """
        self.config = config or SchedulerConfig()
        self.use_genetic = use_genetic
        self.genetic_integration = None
        
        # Initialize genetic integration if enabled
        if self.use_genetic:
            try:
        ...
        self.runner = runner or MonitorRunner(use_genetic=use_genetic)
    ...
    def _cleanup(self) -> None:
        ...
        # Stop genetic evolution if running
        if self.genetic_integration:
            try:
                self.genetic_integration.stop_continuous_evolution()
            except Exception:
                pass
    ...
    def _run_monitor(self) -> RunRecord:
        ...
        try:
            ...
            if self.trade_executor:
                # Reload paper trading state to pick up new strategies / 重新載入模擬交易狀態以偵測新策略
                self.trade_executor.reload_paper()
                    ...
                    for result in results:
                        if result.success and result.current_price:
                            indicators = {
                                ...
                                "atr14": result.atr14,
                                "adx14": result.adx14,
                                "volume_ema20": result.volume_ema20,
                            }
...
def run_every_5_minutes(webhook_url: Optional[str] = None, max_runs: Optional[int] = None, use_genetic: bool = False) -> None:
    """
    ...
    executor = TradeExecutor(exchange=None, position_pct=0.15, max_total_exposure_pct=0.5)
    scheduler = MonitoringScheduler(config=config, notification_channel=channel, trade_executor=executor, use_genetic=use_genetic)
    scheduler.run_interval()


def run_every_5_minutes_with_genetic(webhook_url: Optional[str] = None, max_runs: Optional[int] = None) -> None:
    """
    Run monitoring every 5 minutes with genetic strategies enabled
    每 5 分鐘執行監測，啟用基因策略整合
    """
    run_every_5_minutes(webhook_url=webhook_url, max_runs=max_runs, use_genetic=True)


def run_every_minute(webhook_url: Optional[str] = None, max_runs: Optional[int] = None, use_genetic: bool = False) -> None:
    """
