...
class RuleAction:
    ...
    async def execute(self, context: Dict) -> bool:
        ...
        if self.action_type == "log":
            logging.getLogger(__name__).warning(f"Rule {context.get('rule_id')}: {context.get('message')}")
            return True
        ...
        for rule in self.rules.values():
            ...
            if rule.evaluate(data, self.data_history):
                ...
                if rule.can_alert():
                    ...
                    self.logger.warning(f"Rule triggered: {rule.rule_id} ({rule.name})")
                else:
...
class RuleTemplates:
    ...
    def price_drop_alert(symbol: str, drop_percent: float = 5.0) -> AlertRule:
        ...
        return AlertRule(
            ...
            priority="high",
            conditions=[
                RuleCondition(field="price.change_percent", operator=Operator.DECREASED_BY, value=drop_percent)
            ],
    ...
    def high_volume_alert(symbol: str, volume_multiplier: float = 3.0) -> AlertRule:
        ...
        return AlertRule(
            ...
            priority="medium",
            conditions=[RuleCondition(field="volume.ratio", operator=Operator.GT, value=volume_multiplier)],
            channels=["console"],
    ...
    def balance_low_alert(threshold: float = 100.0) -> AlertRule:
        ...
        return AlertRule(
            ...
            priority="critical",
            conditions=[RuleCondition(field="account.balance", operator=Operator.LT, value=threshold)],
            channels=["console", "log"],
    ...
    def order_failed_alert() -> AlertRule:
        ...
        return AlertRule(
            ...
            priority="high",
            conditions=[RuleCondition(field="order.status", operator=Operator.EQ, value="failed")],
            channels=["console", "log"],
            cooldown_seconds=30,
            category="execution",
            tags=["order", "failure"],
