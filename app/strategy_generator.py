...
class StrategyGenerator:
    ...
    def generate_grid(
        self, strategy_type: StrategyType, symbol: str = "BTCUSDT", timeframe: str = "1h"
    ) -> List[StrategyParams]:
        """
        ...
        for combo in itertools.product(*values):
            ...
            strategy = StrategyParams(strategy_type=strategy_type, symbol=symbol, timeframe=timeframe, params=params)
            strategies.append(strategy)

        self.generated.extend(strategies)

        self.logger.info(f"Generated {len(strategies)} grid combinations " f"for {strategy_type.value}")

        return strategies

    def generate_random(
        self, strategy_type: StrategyType, count: int = 10, symbol: str = "BTCUSDT", timeframe: str = "1h"
    ) -> List[StrategyParams]:
        """
        ...
        self.logger.info(f"Generated {len(strategies)} random combinations " f"for {strategy_type.value}")

        return strategies

    def mutate_strategy(self, strategy: StrategyParams, mutation_rate: float = 0.3) -> StrategyParams:
        """
    ...
    def crossover(self, parent1: StrategyParams, parent2: StrategyParams) -> StrategyParams:
        """
        ...
        for key in parent1.params:
            child_params[key] = (
                parent1.params[key] if random.random() < 0.5 else parent2.params.get(key, parent1.params[key])
            )

        child = StrategyParams(
            strategy_type=parent1.strategy_type, symbol=parent1.symbol, timeframe=parent1.timeframe, params=child_params
        )
...
if __name__ == "__main__":
    ...
    ma_strategies = generator.generate_grid(StrategyType.MA_CROSSOVER, symbol="BTCUSDT", timeframe="4h")

    # Generate random RSI strategies
    rsi_strategies = generator.generate_random(StrategyType.RSI, count=5, symbol="ETHUSDT", timeframe="1h")
