...
class HistoricalCache:
    ...
    def __init__(self, max_candles: int = 10000):
        ...
        self.cache: Dict[str, Dict[str, List[CandleData]]] = defaultdict(lambda: defaultdict(list))
    ...
    def add_candles(self, symbol: str, timeframe: str, candles: List[CandleData]):
        """
        ...
        self.logger.info(
            f"Added {len(candles)} candles: {symbol} {timeframe} " f"(total: {len(self.cache[symbol][timeframe])})"
        )
    ...
    def get_range(self, symbol: str, timeframe: str, lookback: timedelta) -> List[CandleData]:
        """
    ...
    def has_data(self, symbol: str, timeframe: str) -> bool:
        ...
        return (
            symbol.upper() in self.cache
            and timeframe in self.cache[symbol.upper()]
            and len(self.cache[symbol.upper()][timeframe]) > 0
        )

    def get_cache_info(self) -> Dict:
        """Get cache statistics."""
        total_symbols = len(self.cache)
        total_candles = sum(len(candles) for tfs in self.cache.values() for candles in tfs.values())
    ...
    def save_to_disk(self, filepath: str):
        ...
        data = {
            ...
            "cache": {
                symbol: {tf: [c.to_dict() for c in candles] for tf, candles in tfs.items()}
    ...
    def calculate_indicators(self, symbol: str, timeframe: str, indicator: str, period: int = 14) -> List[float]:
        """
