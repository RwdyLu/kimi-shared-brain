...
class Trade:
    ...
    realized_pnl: float
    strategy_id: str
    exit_reason: str = ""  # T-069: track why position was closed / 記錄出場原因
    ...
    def from_dict(cls, data: dict) -> "Trade":
        # Handle legacy trades without exit_reason / 兼容舊交易記錄
...
class PaperTrading:
    ...
    def _load_state(self):
        ...
        Also auto-detects new strategies from config and adds them.
        """
        ...
        # ─── Auto-add new strategies from config / 自動添加新策略 ──
        enabled_ids = self._enabled_strategy_ids()
        added = []
        for sid in enabled_ids:
            if sid not in self.strategies:
                self.strategies[sid] = StrategyAccount(
                    balance=INITIAL_BALANCE_PER_STRATEGY,
                    initial=INITIAL_BALANCE_PER_STRATEGY,
                )
                added.append(sid)
        ...
        if added:
            self.total_initial = len(self.strategies) * INITIAL_BALANCE_PER_STRATEGY
            self._save_state()
            logger.info(f"Auto-added {len(added)} new strategy accounts: {added}")

    def reload(self):
        """Reload state from disk and auto-add new strategies / 重新載入狀態並自動添加新策略"""
        self._load_state()

    def _save_state(self):
        """Save state to disk with file lock / 保存狀態到磁碟（帶文件鎖）"""
        import fcntl
        self.last_updated = datetime.now().isoformat()
        state = {
            "strategies": {
    ...
    def get_total_equity(self, current_prices: Optional[Dict[str, float]] = None) -> float:
        ...
        Args:
            current_prices: Optional dict of symbol -> current price for unrealized PnL calculation
        """
        total = 0.0
        for acc in self.strategies.values():
            position_value = 0.0
            for symbol, positions in acc.positions.items():
                for p in positions:
                    qty = p.get("quantity", 0)
                    # Use current price if available, otherwise fallback to entry_price
                    price = current_prices.get(symbol, 0) if current_prices else 0
                    if price <= 0:
                        price = p.get("entry_price", 0)
                    position_value += qty * price
            total += acc.balance + position_value
        return total
    ...
    def enter_position(
        ...
        trade = {
            ...
            "strategy_id": strategy_id,
            "position_id": position_id,
            "exit_reason": "",  # Set when exit / 出場時填入
        }
