...
class OCOOrderManager:
    ...
    def place_oco(self, oco: OCOOrder, timeout: float = 300.0) -> OCOResult:
        ...
        if not valid:
            return OCOResult(
                success=False, oco_id="", status=OCOStatus.ERROR,
                stop_order_id=None, profit_order_id=None,
                filled_order=None, filled_price=0, filled_quantity=0,
                pnl=0, message=msg, execution_time=0
            )
        
        try:
            self.logger.info(
                f"Placing OCO for {oco.symbol}: "
                f"SL={oco.stop_loss_price}, TP={oco.take_profit_price}"
            )
            ...
            profit_order = self.exchange.create_order(
                ...
                params={
                    'stopPrice': oco.take_profit_price,
                    'timeInForce': oco.time_in_force
                }
            )
            ...
            return OCOResult(
                ...
                stop_order_id=None, profit_order_id=None,
                filled_order=None, filled_price=0, filled_quantity=0,
                pnl=0, message=f"OCO failed: {str(e)}",
    ...
    def _monitor_oco(self, oco_id: str, timeout: float) -> OCOResult:
        ...
        if not oco_info:
            return OCOResult(
                success=False, oco_id=oco_id, status=OCOStatus.ERROR,
                stop_order_id=None, profit_order_id=None,
                filled_order=None, filled_price=0, filled_quantity=0,
                pnl=0, message="OCO not found", execution_time=0
            )
        ...
        while True:
            ...
            if elapsed > timeout:
                ...
                return OCOResult(
                    success=False, oco_id=oco_id, status=OCOStatus.EXPIRED,
                    stop_order_id=stop_id, profit_order_id=profit_id,
                    filled_order=None, filled_price=0, filled_quantity=0,
                    pnl=0, message=f"Timeout after {elapsed:.1f}s",
            ...
            try:
                ...
                if stop_order['status'] == 'closed':
                    ...
                    self.logger.info(
                        f"Stop loss triggered: {filled} @ {avg_price}, PnL: {pnl:.2f}"
                    )
                    
                    del self.active_ocos[oco_id]
                    
                    return OCOResult(
                        success=True, oco_id=oco_id,
                        status=OCOStatus.STOP_TRIGGERED,
                        stop_order_id=stop_id, profit_order_id=profit_id,
                        filled_order='stop', filled_price=avg_price,
                        filled_quantity=filled, pnl=pnl,
                ...
                if profit_order['status'] == 'closed':
                    ...
                    self.logger.info(
                        f"Take profit triggered: {filled} @ {avg_price}, PnL: {pnl:.2f}"
                    )
                    ...
                    return OCOResult(
                        ...
                        stop_order_id=stop_id, profit_order_id=profit_id,
                        filled_order='profit', filled_price=avg_price,
                        filled_quantity=filled, pnl=pnl,
    ...
    def _calculate_pnl(self, side: str, entry: float, exit: float, 
    ...
    def modify_oco(self, oco_id: str, 
                   new_stop: Optional[float] = None,
                   new_profit: Optional[float] = None) -> bool:
        """
...
if __name__ == "__main__":
    ...
    if valid:
        ...
        print(f"  Risk/Reward: 1:{abs(oco.take_profit_price - oco.entry_price) / abs(oco.entry_price - oco.stop_loss_price):.1f}")
