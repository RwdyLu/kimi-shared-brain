...
class RetryConfig:
    ...
    retryable_errors: List[str] = field(default_factory=lambda: [
        "timeout", "connection", "rate_limit", "server_error", "network"
    ])
    non_retryable_errors: List[str] = field(default_factory=lambda: [
        "insufficient_balance", "invalid_symbol", "invalid_amount", "auth_failed"
    ])
...
class AlertManager:
    ...
    def send_alert(self, level: AlertLevel, title: str, message: str,
                  source: str = "order_retry", metadata: Optional[Dict] = None):
        """
...
class OrderRetryManager:
    ...
    async def execute_with_retry(self, operation_id: str,
        ...
        while attempt <= self.config.max_retries:
            ...
            try:
                ...
                self.logger.warning(
                    f"[{operation_id}] Attempt {attempt} failed: {error_msg}. "
    ...
    def get_active_retries(self) -> Dict:
        ...
        return {k: v for k, v in self.active_retries.items() 
                if v["status"] == RetryStatus.RETRYING}
...
async def retry_operation(operation: Callable, *args,
                         max_retries: int = 3,
