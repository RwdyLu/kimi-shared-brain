...
class TaskStateMachine:
    ...
    def transition(self, new_status: TaskStatus, changed_by: str, 
                   reason: str = "") -> bool:
        """
    ...
    def _record_transition(self, old_status: Optional[TaskStatus], 
...
def check_timeout(state_machine: TaskStateMachine) -> Optional[str]:
    ...
    if elapsed > timeout_hours:
        return (
            f"Timeout: {state_machine.status.value} exceeded "
            f"{timeout_hours}h (elapsed: {elapsed:.1f}h)"
        )
