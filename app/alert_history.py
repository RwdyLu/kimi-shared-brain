...
class AlertAnalytics:
    ...
    def resolve_alert(self,
        ...
        for alert in self.alert_history:
            if alert.alert_id == alert_id and not alert.resolved_at:
                ...
                self.logger.info(
                    f"Alert resolved: {alert_id} ({resolution.value}) "
                    f"in {alert.response_time_seconds:.1f}s"
                )
                return True
    ...
    def get_rule_effectiveness(self, days: int = 30) -> Dict[str, Dict]:
        ...
        for rule_id, stats in rule_stats.items():
            ...
            results[rule_id] = {
                ...
                'avg_response_time': sum(stats['avg_response_time']) / len(stats['avg_response_time']) if stats['avg_response_time'] else 0,
    ...
    def get_noise_report(self, days: int = 7) -> Dict:
        ...
        return {
            ...
            ] if noise_rules else ['No noisy rules detected'],
        }
