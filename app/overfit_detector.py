...
class OverfitDetector:
    ...
    def __init__(
        self, degradation_threshold: float = 0.30, mc_pvalue_threshold: float = 0.05, stability_threshold: float = 0.50
    ):
    ...
    def test_insample_degradation(self, insample_return: float, outsample_return: float) -> Tuple[float, bool, str]:
        """
    ...
    def monte_carlo_test(self, returns: List[float], n_permutations: int = 1000) -> Tuple[float, bool, str]:
        """
        Monte Carlo permutation test.
    ...
    def test_parameter_stability(self, param_results: Dict[str, List[float]]) -> Tuple[float, bool, str]:
        """
    ...
    def test_trade_consistency(self, monthly_returns: List[float]) -> Tuple[float, bool, str]:
        """
    ...
    def analyze(
        ...
        param_results: Optional[Dict] = None,
    ) -> OverfitReport:
        """
        ...
        report.degradation, passed, msg = self.test_insample_degradation(insample_return, outsample_return)
        ...
        self.logger.info(
            f"Overfit analysis for {strategy_id}: {report.overall_level.value} " f"({warning_count} warnings)"
        )
