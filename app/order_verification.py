...
class OrderVerifier:
    ...
    def _poll_order(self, order_id: str, symbol: str, start_time: float, timeout: int) -> OrderResult:
        """
        ...
        while True:
            ...
            try:
                ...
                if current_status != last_status:
                    self.logger.info(
                        f"Order {order_id} status: {last_status.value} → " f"{current_status.value} (filled: {filled})"
                    )
                ...
                if current_status == OrderStatus.FILLED:
                    ...
                    self.logger.info(f"Order {order_id} filled: {filled} @ {avg_price} " f"(fee: {fee} {fee_currency})")
