...
class TradeStorage:
    ...
    def add_trade(self, trade: TradeRecord) -> bool:
        ...
        self.logger.info(
            f"Trade added: {trade.trade_id} ({trade.symbol} {trade.side.value} "
            f"@{trade.entry_price})"
        )
    ...
    def close_trade(self, 
                    trade_id: str,
                    exit_price: float,
                    exit_time: Optional[datetime] = None) -> Optional[TradeRecord]:
        """
        ...
        trade.pnl_percent = (trade.realized_pnl / (trade.entry_price * trade.quantity)) * 100 if trade.entry_price else 0
        ...
        self.logger.info(
            f"Trade closed: {trade_id} P&L=${trade.realized_pnl:.2f} "
