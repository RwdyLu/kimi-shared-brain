...
class SNRStrategy:
    ...
    def __init__(self,
                 ...
                 min_touches: int = 2,
                 level_spacing: float = 1.0):  # Minimum % between levels
    ...
    def detect_levels(self,
                     ...
                     highs: List[float],
                     lows: List[float],
                     closes: List[float]) -> List[SNRLevel]:
        """
        ...
        resistance_levels = self._find_extrema(
            highs, window=5, mode='max'
        )
        
        # Find local minima (support)
        support_levels = self._find_extrema(
            lows, window=5, mode='min'
        )
        
        # Cluster nearby levels
        clustered_resistance = self._cluster_levels(
            resistance_levels, closes[-1]
        )
        clustered_support = self._cluster_levels(
            support_levels, closes[-1]
        )
    ...
    def _cluster_levels(self,
                       prices: List[float],
                       current_price: float) -> List[Tuple[float, int]]:
        """
    ...
    def generate_signal(self,
                       symbol: str,
                       current_price: float,
                       levels: Optional[List[SNRLevel]] = None) -> Optional[SNRSignal]:
        """
        ...
        nearest_support = min(
            supports, key=lambda l: abs(l.price - current_price)
        ) if supports else None
        
        nearest_resistance = min(
            resistances, key=lambda l: abs(l.price - current_price)
