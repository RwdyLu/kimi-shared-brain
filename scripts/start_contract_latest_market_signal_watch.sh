...
BLOCKED_PAIRS_SCOPE="${CONTRACT_MARKET_BLOCKED_PAIRS_SCOPE:-current_policy}"
...
FAST_SHADOW_RETEST_MARKER="${CONTRACT_MARKET_FAST_SHADOW_RETEST_MARKER:-state/FOUND_CURRENT_POLICY_FAST_SHADOW_RETEST.txt}"
FAST_SHADOW_NO_RETEST_MARKER="${CONTRACT_MARKET_FAST_SHADOW_NO_RETEST_MARKER:-state/NO_CURRENT_POLICY_FAST_SHADOW_RETEST.txt}"
FAST_SHADOW_RETEST_JSON="${CONTRACT_MARKET_FAST_SHADOW_RETEST_JSON:-state/current_policy_fast_shadow_retest.json}"
...
DATA_FRESHNESS_MODE="${CONTRACT_MARKET_DATA_FRESHNESS_MODE:-block}"
DATA_FRESHNESS_MAX_AGE_BARS="${CONTRACT_MARKET_DATA_FRESHNESS_MAX_AGE_BARS:-2.5}"
DATA_FRESHNESS_GRACE_MINUTES="${CONTRACT_MARKET_DATA_FRESHNESS_GRACE_MINUTES:-5.0}"
JOURNAL_MAX_ACTIVE_PER_PAIR="${CONTRACT_MARKET_JOURNAL_MAX_ACTIVE_PER_PAIR:-1}"
JOURNAL_RISK_SCOPE="${CONTRACT_MARKET_JOURNAL_RISK_SCOPE:-current_policy}"
JOURNAL_ACTIVE_CAP_SCOPE="${CONTRACT_MARKET_JOURNAL_ACTIVE_CAP_SCOPE:-current_policy}"
JOURNAL_RECORD_MODE="${CONTRACT_MARKET_JOURNAL_RECORD_MODE:-strong_analog}"
JOURNAL_STRONG_MIN_ANALOG_SAMPLES="${CONTRACT_MARKET_JOURNAL_STRONG_MIN_ANALOG_SAMPLES:-30}"
JOURNAL_STRONG_MIN_EXPECTANCY_R="${CONTRACT_MARKET_JOURNAL_STRONG_MIN_EXPECTANCY_R:-0.25}"
JOURNAL_STRONG_MIN_HIT_RATE="${CONTRACT_MARKET_JOURNAL_STRONG_MIN_HIT_RATE:-0.50}"
JOURNAL_STRONG_MIN_PROFITABLE_RATE="${CONTRACT_MARKET_JOURNAL_STRONG_MIN_PROFITABLE_RATE:-0.55}"
JOURNAL_STRONG_REQUIRE_ROBUSTNESS="${CONTRACT_MARKET_JOURNAL_STRONG_REQUIRE_ROBUSTNESS:-1}"
...
JOURNAL_SHADOW_RETEST_JSON="${CONTRACT_MARKET_JOURNAL_SHADOW_RETEST_JSON:-$ACTIONS_JSON}"
JOURNAL_SHADOW_RETEST_MAX_NEW="${CONTRACT_MARKET_JOURNAL_SHADOW_RETEST_MAX_NEW:-2}"
ANALOG_MIN_ROBUST_SEGMENTS="${CONTRACT_MARKET_ANALOG_MIN_ROBUST_SEGMENTS:-2}"
ANALOG_ROBUST_SEGMENT_MIN_SAMPLES="${CONTRACT_MARKET_ANALOG_ROBUST_SEGMENT_MIN_SAMPLES:-3}"
MIN_ANALOG_SEGMENT_EXPECTANCY_R="${CONTRACT_MARKET_MIN_ANALOG_SEGMENT_EXPECTANCY_R:-0.0}"
MIN_ANALOG_SEGMENT_PROFITABLE_RATE="${CONTRACT_MARKET_MIN_ANALOG_SEGMENT_PROFITABLE_RATE:-0.45}"
...
MARKET_QUALITY_MODE="${CONTRACT_MARKET_QUALITY_MODE:-block}"
MARKET_QUALITY_MIN_ATR_PCT="${CONTRACT_MARKET_QUALITY_MIN_ATR_PCT:-0.0015}"
MARKET_QUALITY_MAX_ATR_PCT="${CONTRACT_MARKET_QUALITY_MAX_ATR_PCT:-0.08}"
MARKET_QUALITY_MAX_EMA_GAP="${CONTRACT_MARKET_QUALITY_MAX_EMA_GAP:-0.15}"
MARKET_QUALITY_MAX_ABS_RET_24H="${CONTRACT_MARKET_QUALITY_MAX_ABS_RET_24H:-0.20}"
MARKET_QUALITY_MAX_BREAKOUT_EXTENSION="${CONTRACT_MARKET_QUALITY_MAX_BREAKOUT_EXTENSION:-0.35}"
MARKET_QUALITY_MIN_VOLUME_RATIO="${CONTRACT_MARKET_QUALITY_MIN_VOLUME_RATIO:-0.25}"
MARKET_QUALITY_VOLUME_WINDOW="${CONTRACT_MARKET_QUALITY_VOLUME_WINDOW:-48}"

if [[ "$SESSION" == -* ]]; then
  echo "session name must not start with '-'" >&2
  exit 2
fi
if ! [[ "$TOP_N" =~ ^[0-9]+$ && "$SLEEP_SEC" =~ ^[0-9]+$ && "$LOOKBACK_BARS_IF_EMPTY" =~ ^[0-9]+$ && "$ANALOG_HORIZON_BARS" =~ ^[0-9]+$ && "$PAPER_OUTCOME_HORIZON_BARS" =~ ^[0-9]+$ && "$FAST_SHADOW_OUTCOME_HORIZON_BARS" =~ ^[0-9]+$ && "$PAPER_ENTRY_LATENCY_BARS" =~ ^[0-9]+$ && "$JOURNAL_MAX_ACTIVE_PER_PAIR" =~ ^[0-9]+$ && "$JOURNAL_STRONG_MIN_ANALOG_SAMPLES" =~ ^[0-9]+$ && "$JOURNAL_SHADOW_MIN_ANALOG_SAMPLES" =~ ^[0-9]+$ && "$JOURNAL_SHADOW_RETEST_MAX_NEW" =~ ^[0-9]+$ && "$ANALOG_MIN_ROBUST_SEGMENTS" =~ ^[0-9]+$ && "$ANALOG_ROBUST_SEGMENT_MIN_SAMPLES" =~ ^[0-9]+$ && "$REGIME_MIN_DIRECTION_VOTES" =~ ^[0-9]+$ && "$REGIME_VOL_LOOKBACK_BARS" =~ ^[0-9]+$ && "$MARKET_QUALITY_VOLUME_WINDOW" =~ ^[0-9]+$ ]]; then
  echo "CONTRACT_MARKET_TOP_N, CONTRACT_MARKET_SIGNAL_SLEEP_SEC, CONTRACT_MARKET_LOOKBACK_BARS_IF_EMPTY, CONTRACT_MARKET_ANALOG_HORIZON_BARS, CONTRACT_MARKET_PAPER_OUTCOME_HORIZON_BARS, CONTRACT_MARKET_FAST_SHADOW_OUTCOME_HORIZON_BARS, CONTRACT_MARKET_PAPER_ENTRY_LATENCY_BARS, CONTRACT_MARKET_JOURNAL_MAX_ACTIVE_PER_PAIR, CONTRACT_MARKET_JOURNAL_STRONG_MIN_ANALOG_SAMPLES, CONTRACT_MARKET_JOURNAL_SHADOW_MIN_ANALOG_SAMPLES, CONTRACT_MARKET_JOURNAL_SHADOW_RETEST_MAX_NEW, CONTRACT_MARKET_ANALOG_MIN_ROBUST_SEGMENTS, CONTRACT_MARKET_ANALOG_ROBUST_SEGMENT_MIN_SAMPLES, CONTRACT_MARKET_REGIME_MIN_DIRECTION_VOTES, CONTRACT_MARKET_REGIME_VOL_LOOKBACK_BARS, and CONTRACT_MARKET_QUALITY_VOLUME_WINDOW must be integers" >&2
  exit 2
fi
...
case "$JOURNAL_RECORD_MODE" in
  all_signals|analog_supported|strong_analog|off) ;;
  *)
    echo "CONTRACT_MARKET_JOURNAL_RECORD_MODE must be all_signals, analog_supported, strong_analog, or off" >&2
    exit 2
    ;;
esac
case "$DATA_FRESHNESS_MODE" in
  off|annotate|block) ;;
  *)
    echo "CONTRACT_MARKET_DATA_FRESHNESS_MODE must be off, annotate, or block" >&2
    exit 2
    ;;
esac
case "$JOURNAL_RISK_SCOPE" in
  current_policy|global) ;;
  *)
    echo "CONTRACT_MARKET_JOURNAL_RISK_SCOPE must be current_policy or global" >&2
    exit 2
    ;;
esac
case "$JOURNAL_ACTIVE_CAP_SCOPE" in
  current_policy|all) ;;
  *)
    echo "CONTRACT_MARKET_JOURNAL_ACTIVE_CAP_SCOPE must be current_policy or all" >&2
    exit 2
    ;;
esac
case "$BLOCKED_PAIRS_SCOPE" in
  current_policy|global) ;;
  *)
    echo "CONTRACT_MARKET_BLOCKED_PAIRS_SCOPE must be current_policy or global" >&2
    exit 2
    ;;
esac
...
case "$MARKET_QUALITY_MODE" in
  off|annotate|block) ;;
  *)
    echo "CONTRACT_MARKET_QUALITY_MODE must be off, annotate, or block" >&2
    exit 2
    ;;
esac
...
printf -v FAST_SHADOW_RETEST_MARKER_Q "%q" "$FAST_SHADOW_RETEST_MARKER"
printf -v FAST_SHADOW_NO_RETEST_MARKER_Q "%q" "$FAST_SHADOW_NO_RETEST_MARKER"
printf -v FAST_SHADOW_RETEST_JSON_Q "%q" "$FAST_SHADOW_RETEST_JSON"
...
printf -v DATA_FRESHNESS_MODE_Q "%q" "$DATA_FRESHNESS_MODE"
printf -v DATA_FRESHNESS_MAX_AGE_BARS_Q "%q" "$DATA_FRESHNESS_MAX_AGE_BARS"
printf -v DATA_FRESHNESS_GRACE_MINUTES_Q "%q" "$DATA_FRESHNESS_GRACE_MINUTES"
printf -v JOURNAL_MAX_ACTIVE_Q "%q" "$JOURNAL_MAX_ACTIVE_PER_PAIR"
printf -v JOURNAL_RISK_SCOPE_Q "%q" "$JOURNAL_RISK_SCOPE"
printf -v JOURNAL_ACTIVE_CAP_SCOPE_Q "%q" "$JOURNAL_ACTIVE_CAP_SCOPE"
printf -v JOURNAL_RECORD_MODE_Q "%q" "$JOURNAL_RECORD_MODE"
printf -v JOURNAL_STRONG_MIN_ANALOG_Q "%q" "$JOURNAL_STRONG_MIN_ANALOG_SAMPLES"
printf -v JOURNAL_STRONG_MIN_EXPECTANCY_Q "%q" "$JOURNAL_STRONG_MIN_EXPECTANCY_R"
printf -v JOURNAL_STRONG_MIN_HIT_Q "%q" "$JOURNAL_STRONG_MIN_HIT_RATE"
printf -v JOURNAL_STRONG_MIN_PROFITABLE_Q "%q" "$JOURNAL_STRONG_MIN_PROFITABLE_RATE"
printf -v BLOCKED_PAIRS_SCOPE_Q "%q" "$BLOCKED_PAIRS_SCOPE"
...
printf -v JOURNAL_SHADOW_RETEST_JSON_Q "%q" "$JOURNAL_SHADOW_RETEST_JSON"
printf -v JOURNAL_SHADOW_RETEST_MAX_NEW_Q "%q" "$JOURNAL_SHADOW_RETEST_MAX_NEW"
printf -v ANALOG_MIN_ROBUST_SEGMENTS_Q "%q" "$ANALOG_MIN_ROBUST_SEGMENTS"
printf -v ANALOG_ROBUST_SEGMENT_MIN_SAMPLES_Q "%q" "$ANALOG_ROBUST_SEGMENT_MIN_SAMPLES"
printf -v MIN_ANALOG_SEGMENT_EXPECTANCY_Q "%q" "$MIN_ANALOG_SEGMENT_EXPECTANCY_R"
printf -v MIN_ANALOG_SEGMENT_PROFITABLE_Q "%q" "$MIN_ANALOG_SEGMENT_PROFITABLE_RATE"
...
printf -v MARKET_QUALITY_MODE_Q "%q" "$MARKET_QUALITY_MODE"
printf -v MARKET_QUALITY_MIN_ATR_Q "%q" "$MARKET_QUALITY_MIN_ATR_PCT"
printf -v MARKET_QUALITY_MAX_ATR_Q "%q" "$MARKET_QUALITY_MAX_ATR_PCT"
printf -v MARKET_QUALITY_MAX_EMA_GAP_Q "%q" "$MARKET_QUALITY_MAX_EMA_GAP"
printf -v MARKET_QUALITY_MAX_RET_Q "%q" "$MARKET_QUALITY_MAX_ABS_RET_24H"
printf -v MARKET_QUALITY_MAX_BREAKOUT_Q "%q" "$MARKET_QUALITY_MAX_BREAKOUT_EXTENSION"
printf -v MARKET_QUALITY_MIN_VOLUME_Q "%q" "$MARKET_QUALITY_MIN_VOLUME_RATIO"
printf -v MARKET_QUALITY_VOLUME_WINDOW_Q "%q" "$MARKET_QUALITY_VOLUME_WINDOW"
...
echo "analog_robustness: segments=$ANALOG_MIN_ROBUST_SEGMENTS segment_min_samples=$ANALOG_ROBUST_SEGMENT_MIN_SAMPLES min_segment_expectancy=$MIN_ANALOG_SEGMENT_EXPECTANCY_R min_segment_profitable=$MIN_ANALOG_SEGMENT_PROFITABLE_RATE"
...
echo "data_freshness: mode=$DATA_FRESHNESS_MODE max_age_bars=$DATA_FRESHNESS_MAX_AGE_BARS grace_minutes=$DATA_FRESHNESS_GRACE_MINUTES"
...
echo "market_quality: mode=$MARKET_QUALITY_MODE atr_pct=$MARKET_QUALITY_MIN_ATR_PCT..$MARKET_QUALITY_MAX_ATR_PCT max_ema_gap=$MARKET_QUALITY_MAX_EMA_GAP max_abs_ret_24h=$MARKET_QUALITY_MAX_ABS_RET_24H max_breakout_extension=$MARKET_QUALITY_MAX_BREAKOUT_EXTENSION min_volume_ratio=$MARKET_QUALITY_MIN_VOLUME_RATIO volume_window=$MARKET_QUALITY_VOLUME_WINDOW"
...
echo "journal_strong_gate: samples=$JOURNAL_STRONG_MIN_ANALOG_SAMPLES expectancy=$JOURNAL_STRONG_MIN_EXPECTANCY_R hit=$JOURNAL_STRONG_MIN_HIT_RATE profitable=$JOURNAL_STRONG_MIN_PROFITABLE_RATE require_robustness=$JOURNAL_STRONG_REQUIRE_ROBUSTNESS"
...
echo "journal_shadow_retest: json=$JOURNAL_SHADOW_RETEST_JSON max_new=$JOURNAL_SHADOW_RETEST_MAX_NEW"
echo "blocked_pairs_json: $BLOCKED_PAIRS_JSON scope=$BLOCKED_PAIRS_SCOPE"
...
echo "fast_shadow_retest_marker: $FAST_SHADOW_RETEST_MARKER"
echo "fast_shadow_no_retest_marker: $FAST_SHADOW_NO_RETEST_MARKER"
echo "fast_shadow_retest_json: $FAST_SHADOW_RETEST_JSON"
