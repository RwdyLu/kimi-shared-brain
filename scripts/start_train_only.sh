...
PLANNER_PRESET_MODE="${PLANNER_PRESET_MODE:-xsec_first}"
...
tmux new-session -d -s "$SESSION" \
  "cd $ROOT_Q && python3 -m v9.contract.auto_research --mode continuous --continue-after-candidate --target-distinct-candidates $TARGET_DISTINCT_CANDIDATES --planner-batch-size $PLANNER_BATCH_SIZE --planner-preset-mode $PLANNER_PRESET_MODE --max-cycles $MAX_CYCLES --max-hours $MAX_HOURS --cycle-sleep-sec $CYCLE_SLEEP_SEC --idle-backoff-initial-sec $IDLE_BACKOFF_INITIAL_SEC --idle-backoff-max-sec $IDLE_BACKOFF_MAX_SEC --idle-poll-sec $IDLE_POLL_SEC >> $LOG_Q 2>&1"
