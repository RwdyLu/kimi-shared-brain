...
class SignalEngine:
    ...
    def _check_ichimoku_conditions(
        ...
        """
        if len(closes) < 26:
            return False, None, "Not enough data (need 26 periods)"
        ...
        # Long: close above cloud (relaxed: no longer requires tenkan > kijun)
        if current_close > cloud_top:
            return True, "LONG", f"Ichimoku LONG: close {current_close:.2f} > cloud {cloud_top:.2f}"

        # Short: close below cloud (relaxed: no longer requires tenkan < kijun)
        if current_close < cloud_bottom:
            return True, "SHORT", f"Ichimoku SHORT: close {current_close:.2f} < cloud {cloud_bottom:.2f}"
    ...
    def _check_williams_r_conditions(
        ...
        # Long: WR crosses above -70 from below (oversold bounce)
        if prev_wr < -70 and current_wr >= -70:
            return True, "LONG", f"Williams %R oversold bounce: {prev_wr:.1f} -> {current_wr:.1f}"

        # Short: WR crosses below -30 from above (overbought pullback)
        if prev_wr > -30 and current_wr <= -30:
    ...
    def _check_rsi_mid_bounce_conditions(
        ...
        closes: List[float],
        period: int = 14
    ) -> Tuple[bool, str, str, List[float]]:
        """
        Check RSI mid bounce conditions / 檢查 RSI 中間反彈條件
        從收盤價即時計算 RSI，不依賴預先計算的 RSI 資料

        Returns: (triggered, direction, reason, rsi_values)
        """
        if len(closes) < period + 1:
            return False, None, f"Not enough data (need {period + 1} closes)", []

        from indicators.calculator import calculate_rsi
        rsi_values = calculate_rsi(closes, period)
        if not rsi_values or len(rsi_values) < 2:
            return False, None, "RSI calculation failed", rsi_values

        prev_rsi = rsi_values[-2]
        curr_rsi = rsi_values[-1]

        if prev_rsi < 45 and curr_rsi >= 50:
            return True, "LONG", f"RSI mid bounce LONG: {prev_rsi:.1f} -> {curr_rsi:.1f}", rsi_values
        elif prev_rsi > 55 and curr_rsi <= 50:
            return True, "SHORT", f"RSI mid bounce SHORT: {prev_rsi:.1f} -> {curr_rsi:.1f}", rsi_values

        return False, None, f"RSI mid bounce: curr={curr_rsi:.1f}, prev={prev_rsi:.1f}", rsi_values
    ...
    def generate_rsi_mid_bounce_signal(
        ...
        try:
            triggered, direction, reason, rsi_values = self._check_rsi_mid_bounce_conditions(closes)
        except Exception:
            return None
    ...
    def _check_momentum_divergence_conditions(
        ...
        從收盤價即時計算動量，不依賴預先計算的動量資料

        Returns: (triggered, direction, reason)
        """
        if len(closes) < momentum_period + 5:
            return False, None, f"Not enough data (need {momentum_period + 5} closes)"

        # Calculate simple momentum: close[t] - close[t - momentum_period]
        momentum = []
        for i in range(momentum_period, len(closes)):
            momentum.append(closes[i] - closes[i - momentum_period])

        if len(momentum) < 5:
            return False, None, "Not enough momentum data"
    ...
    def generate_momentum_divergence_signal(
        ...
        triggered, direction, reason = self._check_momentum_divergence_conditions(closes)
        if not triggered:
            return None
    ...
    def process_symbol(
        ...
        # Generate ema_cross_fast signal / 產生 EMA 快速交叉訊號
        if closes_5m and len(closes_5m) >= 10:
            ema_cross_fast = self.generate_ema_cross_fast_signal(
                symbol=symbol,
                closes=closes_5m
            )
            if ema_cross_fast:
                signals.append(ema_cross_fast)

        # Generate rsi_mid_bounce signal / 產生 RSI 中間反彈訊號
        if closes_5m and len(closes_5m) >= 15:
            rsi_mid_bounce = self.generate_rsi_mid_bounce_signal(
                symbol=symbol,
                closes=closes_5m
            )
            if rsi_mid_bounce:
                signals.append(rsi_mid_bounce)

        # Generate price_channel_break signal / 產生價格通道突破訊號
        if highs_5m and lows_5m and closes_5m and len(closes_5m) >= 10:
            price_channel_break = self.generate_price_channel_break_signal(
                symbol=symbol,
                highs=highs_5m,
                lows=lows_5m,
                closes=closes_5m
            )
            if price_channel_break:
                signals.append(price_channel_break)

        # Generate momentum_divergence signal / 產生動量背離訊號
        if closes_5m and len(closes_5m) >= 15:
            momentum_divergence = self.generate_momentum_divergence_signal(
                symbol=symbol,
                closes=closes_5m
            )
        ...
        # Generate opening_range_breakout signal / 產生開盤區間突破訊號
        opens_5m = [k.open for k in data_5m]
        if opens_5m and highs_5m and lows_5m and closes_5m and len(closes_5m) >= 5:
            opening_range_breakout = self.generate_opening_range_breakout_signal(
                symbol=symbol,
                opens=opens_5m,
                highs=highs_5m,
                lows=lows_5m,
    ...
    # =========================================================================
    # Composite EMA+RSI+Volume Strategy (Phase 3 — Freqtrade Port)
    # =========================================================================

    def generate_composite_ema_rsi_volume_signal(
        self,
        symbol: str,
        closes: List[float],
        ...
        """
        Generate composite EMA+RSI+Volume signal.
        Delegates to signals.strategies.ema_rsi_volume for evaluation.
        """
        try:
            from signals.strategies.ema_rsi_volume import evaluate
            signal = evaluate(
                symbol=symbol,
                closes=closes,
                volumes=volumes,
    ...
    def generate_signals(self, indicators: Dict[str, Any]) -> List[Signal]:
        ...
        # RSI mid bounce from closes / 從收盤價計算 RSI
        if closes_list and len(closes_list) >= 15:
            try:
                rsi_signal = self.generate_rsi_mid_bounce_signal("BACKTEST", closes_list)
                if rsi_signal:
        ...
        if highs_list and lows_list and closes_list and len(closes_list) >= 10:
            try:
        ...
        # Momentum divergence from closes / 從收盤價計算動量
        if closes_list and len(closes_list) >= 15:
            try:
                md_signal = self.generate_momentum_divergence_signal("BACKTEST", closes_list)
                if md_signal:
        ...
        if closes_list and len(closes_list) >= 30:
            try:
                volumes_list = indicators.get('volumes') or indicators.get('volume_history')
                comp_long = self.generate_composite_ema_rsi_volume_signal(
                    symbol="BACKTEST",
                    closes=closes_list,
                    volumes=volumes_list,
                    direction="long",
                    parameters=indicators.get("composite_params"),
                )
                if comp_long:
                    signals.append(comp_long)
                comp_short = self.generate_composite_ema_rsi_volume_signal(
                    symbol="BACKTEST",
                    closes=closes_list,
                    volumes=volumes_list,
                    direction="short",
