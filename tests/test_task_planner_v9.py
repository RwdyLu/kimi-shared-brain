...
def test_xsec_first_preset_mode_prioritizes_xsec_without_disabling_tsmom() -> None:
    ...
    assert [task.preset for task in first[:17]] == [
        "evergreen_regime_guarded",
        "evergreen_guarded",
        "evergreen_fast",
        "breakout_fast",
    ...
    assert all(task.module == "v9.contract.xsec_ohlcv_factory" for task in first[:17])
    assert [task.preset for task in first[17:20]] == [
        "tsmom_defensive_regime",
        "tsmom_bear_short_regime",
        "tsmom_trend_ensemble",
    ]
    assert all(task.module == "v9.contract.tsmom_factory" for task in first[17:20])
