...
def test_xsec_first_preset_mode_prioritizes_xsec_without_disabling_tsmom() -> None:
    ...
    assert [task.preset for task in first[:19]] == [
        "evergreen_regime_guarded",
        "evergreen_lowvol_guarded",
        "evergreen_guarded",
        "evergreen_fast",
    ...
    assert all(task.module == "v9.contract.xsec_ohlcv_factory" for task in first[:19])
    assert [task.preset for task in first[19:20]] == [
        "tsmom_defensive_regime",
    ]
    assert all(task.module == "v9.contract.tsmom_factory" for task in first[19:20])
