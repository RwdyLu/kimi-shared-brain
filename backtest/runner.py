...
class BacktestRunner:
    ...
    def run(self) -> BacktestSummary:
        ...
        # Close any remaining open trades at end of test — use last close price
        last_price = float(df['close'].iloc[-1]) if len(df) > 0 else None
        self._close_all_trades("end_of_test", last_price=last_price)
    ...
    def _close_all_trades(self, reason: str, last_price: Optional[float] = None) -> None:
        """
        Close all active trades at end of backtest
        回測結束時平倉所有持倉 — 使用最後收盤價計算實際盈虧
        """
        for symbol in list(self.active_trades.keys()):
            trade = self.active_trades[symbol]
            exit_price = last_price if last_price is not None else trade.entry_price
            self._close_trade(symbol, "end_of_test", exit_price, reason)
